...
WITH numbered_instruments AS (
    SELECT id, asset_class, ROW_NUMBER() OVER (ORDER BY id) - 1 AS rn
    FROM instruments
),
numbered_counterparties AS (
...
SELECT
    ...
    inst.asset_class                                          AS asset_class,
...
JOIN numbered_instruments inst ON inst.rn = (n % 50)
JOIN numbered_counterparties cp ON cp.rn = (n % 10);
