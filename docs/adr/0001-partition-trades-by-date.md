...
- Single large unpartitioned table with B-tree index on trade_date
- Partitioning by surrogate ID range

Constraints / forces:
- 50,000 trades per day, keeping 5 years of history (91M rows)
- Most queries are date-range filtered
- Date-range deletes/archivals must be fast
